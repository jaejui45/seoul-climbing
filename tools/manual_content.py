...
DOC = [
 ...
 ("h1", "4. 암장 데이터"),
 ("p", "서울 클라이밍장 104곳(실내 93, 야외 인공암벽 11)이 들어 있습니다. "
       "출처는 카카오맵이고, 업주가 직접 등록한 정보라 요일별 영업시간과 공휴일 영업시간까지 담깁니다."),
 ("warn", "web/gyms.json 을 직접 고치지 마세요. sync-data.sh 를 실행하면 덮어써집니다. "
          "원본은 프로젝트 루트의 gyms.json 이고, 그것도 collect.py 가 만듭니다."),
 ...
 ("h2", "데이터 새로 받기"),
 ("p", "암장이 새로 생기거나 영업시간이 바뀌었을 때 실행합니다. 수 분 걸립니다."),
 ("code", ["cd ~/Developer/SeoulClimb",
           "./sync-data.sh --collect"]),
 ("p", "collect.py 가 카카오맵에서 25개 구를 훑어 장소를 모으고, 각 장소의 상세 정보를 받아 "
       "서울 외 지역과 중복을 걸러낸 뒤 gyms.json 을 새로 씁니다. "
       "이어서 web/ 과 iOS 앱에 자동으로 복사됩니다."),
 ("note", "--collect 없이 ./sync-data.sh 만 실행하면 수집은 건너뛰고 복사만 합니다. "
          "gyms.json 을 손으로 조금 고친 뒤 반영할 때 씁니다."),

 ("h2", "특정 암장 정보를 고치고 싶을 때"),
 ("p", "카카오맵 정보가 틀렸다면 collect.py 안의 표를 고칩니다."),
 ("kv", ("", [
